...
def process_command():
    ...
    if is_music_command:
        ...
        try:
            extraction_prompt = f"Извлеки из этого запроса исполнителя и название песни, и верни только чистый поисковый запрос в формате 'Исполнитель - Название'. Запрос: '{command}'"
            response = model.generate_content(extraction_prompt)
            query = response.text.strip()
            
            logging.info(f"Уточненный поисковый запрос: '{query}'")
            ...
            response_text = f"Включаю песенку: {title} от {artist}! 🎶"
            return jsonify({
                "type": "music",
                "videoId": video_id,
                "title": title,
        ...
        try:
            ...
            return jsonify({"type": "talk", "text": response.text.strip()})
        except Exception as e:
